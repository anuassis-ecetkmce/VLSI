...
class cfs_apb_trans extends uvm_sequence_item;
	...
	// Constraints (if any)
	constraint addr_c { addr inside {[0:2**`CFS_APB_ADDR_WIDTH-1]}; }
	constraint wdata_c { wdata inside {[0:2**`CFS_APB_DATA_WIDTH-1]}; }
	...
	function string convert2string();
		result = $sformatf("APB_TRANS: addr=0x%0h dir=%s wstrb=%0b",
			addr, direction.name(), write_strobe);
        return result;
